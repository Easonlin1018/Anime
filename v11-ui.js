(function () {
    ...
    function renderStats(){const s=V.watchStats(watchHistory,animeList);const labels=[["今天觀看",s.today],["本週觀看",s.week],["本月觀看",s.month],["總觀看",s.total],["完成作品",s.completed],["正在觀看",s.watching],["平均評分",s.averageRating],["連續天數",s.streak],["常用平台",s.topPlatform],["常用標籤",s.topTag]];const bars=data=>`<div class="v11-chart-wrap"><div class="v11-trend">${data.map(x=>`<div class="v11-bar" style="height:${Math.max(3,x.count*10)}px" title="${x.date}: ${x.count}"></div>`).join("")}</div></div>`;document.getElementById("v11-stats").innerHTML=`<div class="v11-grid v11-stats-grid">${labels.map(([a,b])=>`<div class="v11-card v11-stat">${a}<strong class="v11-stat-value">${escapeHtml(b)}</strong></div>`).join("")}</div><h3>最近 7 天</h3>${bars(s.trend7)}<h3>最近 30 天</h3>${bars(s.trend30)}`;}
