(function () {
    ...
    function snapshot() { return { animeList: V.migrateList(animeList), eventOverrides: { ...eventOverrides }, settings: currentSettings(), watchHistory: [...watchHistory], reminders: {}, preferences: parse(PREFS_KEY, {}) }; }
    ...
    updateProgress = function (id, delta) {
        const item = animeList.find(x => String(x.id) === String(id));
        if (!item) return;
        originalUpdateProgress(id, delta);
        const updated = animeList.find(x => String(x.id) === String(id));
        if (updated) {
            updated.currentEpisode = Number(updated.watched) || 0;
            updated.lastWatchedAt = Number(delta) > 0 ? new Date().toISOString() : updated.lastWatchedAt;
            touch(updated);
            watchHistory.push(V.createWatchRecord(updated, delta));
            ...
            persistAnime(); originalRenderList(); refreshV11();
        }
    };
    ...
    buildAnimeItem = function (anime) {
        const html = originalBuildAnimeItem(anime);
    ...
    function statsPage() { return '<section class="section v11-page" data-v11-page-group="stats" hidden><div class="section-title">📊 觀看統計</div><div class="tiny-note">觀看統計自 v11.0 啟用後開始記錄</div><div id="v11-stats"></div></section>'; }
    ...
    function closeV11Modals(){ document.querySelectorAll(".v11-modal").forEach(x=>x.hidden=true); }
    ...
    function renderReminders() { const now=new Date(), week=new Date(now); week.setDate(week.getDate()+7); const list=animeList.filter(x=>x.reminderEnabled&&x.nextEpisodeAt).sort((a,b)=>Date.parse(a.nextEpisodeAt)-Date.parse(b.nextEpisodeAt)); document.getElementById("v11-reminders").innerHTML=list.map(item=>{const date=new Date(item.nextEpisodeAt), diff=date-now, due=diff<=0; const days=Math.floor(Math.abs(diff)/86400000), hours=Math.floor(Math.abs(diff)%86400000/3600000), label=due?"已播出・待看":diff<86400000?"今天更新":diff<172800000?"明天更新":date<=week?"本週更新":"稍後";return `<div class="v11-card"><span class="v11-badge ${due?"danger":"good"}">${label}</span><strong>${escapeHtml(item.title)}</strong><div>下一集：${escapeHtml(date.toLocaleString("zh-TW"))}</div><div>${due?"已過":"剩餘"}：${days} 天 ${hours} 小時</div></div>`}).join("")||'<div class="empty">目前沒有啟用提醒的作品。</div>'; }
    ...
    function renderStats(){const s=V.watchStats(watchHistory,animeList);const labels=[["今天觀看",s.today],["本週觀看",s.week],["本月觀看",s.month],["總觀看",s.total],["完成作品",s.completed],["正在觀看",s.watching],["平均評分",s.averageRating],["連續天數",s.streak],["常用平台",s.topPlatform],["常用標籤",s.topTag]];const bars=data=>`<div class="v11-trend">${data.map(x=>`<div class="v11-bar" style="height:${Math.max(3,x.count*10)}px" title="${x.date}: ${x.count}"></div>`).join("")}</div>`;document.getElementById("v11-stats").innerHTML=`<div class="v11-grid">${labels.map(([a,b])=>`<div class="v11-card v11-stat">${a}<strong>${escapeHtml(b)}</strong></div>`).join("")}</div><h3>最近 7 天</h3>${bars(s.trend7)}<h3>最近 30 天</h3>${bars(s.trend30)}`;}
    ...
    function readImport(event){const file=event.target.files[0];event.target.value="";if(!file)return;if(file.size>10*1024*1024)return showToast("匯入檔案不可超過 10 MB");const reader=new FileReader();reader.onload=()=>{try{const validation=V.validateBackup(reader.result);if(!validation.valid)throw Error(validation.error);pendingImport=validation.normalized;const p=validation.preview;document.getElementById("v11-import-preview").textContent=`備份格式：${p.format}｜版本：${p.schemaVersion}｜匯出：${p.exportedAt||"未知"}｜找到動漫：${p.animeCount} 部｜活動修正：${p.overrideCount}｜觀看紀錄：${p.historyCount}${p.requiresEmptyConfirmation?"｜⚠️ 備份內含 0 部動漫，套用前會再次確認":""}`;document.getElementById("v11-import-modal").hidden=false}catch(error){pendingImport=null;showToast(`匯入失敗：${escapeHtml(error.message)}`)}};reader.onerror=()=>{pendingImport=null;showToast("匯入失敗：無法讀取檔案")};reader.readAsText(file);}
    function applyImport(mode){if(!pendingImport)return;const count=pendingImport[V.STORAGE_KEY].length;if(count===0&&!confirm("此備份包含 0 部動漫。確定仍要繼續匯入？"))return;if(mode==="replace"&&!confirm("完全覆蓋會取代目前資料。系統會先建立還原點，確定繼續？"))return;let result;try{result=V.importBackup(snapshot(),pendingImport,mode)}catch(error){showToast(`匯入失敗：${escapeHtml(error.message)}`);return}createRestorePoint("匯入前自動還原點");animeList=result.animeList;eventOverrides=result.eventOverrides;watchHistory=result.watchHistory;state.lastImportRestore=parse(V.RESTORE_KEY,[])[0]?.id;save(V.HISTORY_KEY,watchHistory);save(EVENT_OVERRIDES_KEY,eventOverrides);persistAnime();renderList();renderEvents();closeV11Modals();showToast(`匯入完成：已處理 ${result.importedAnimeCount} 部動漫`);}
    ...
    function restorePoint(id){const point=parse(V.RESTORE_KEY,[]).find(x=>x.id===id);if(!point||!confirm(`確定還原 ${new Date(point.createdAt).toLocaleString("zh-TW")} 的資料？目前資料會先建立還原點。`))return;createRestorePoint("還原前自動備份");const result=V.importBackup(snapshot(),point.data,"replace");animeList=result.animeList;eventOverrides=result.eventOverrides;watchHistory=result.watchHistory;save(V.HISTORY_KEY,watchHistory);save(EVENT_OVERRIDES_KEY,eventOverrides);persistAnime();renderList();renderEvents();showToast("還原完成");}
