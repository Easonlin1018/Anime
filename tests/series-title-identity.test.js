...
test("20. Service Worker 離線快取固定版 OpenCC 與 series graph cache", () => {
    ...
    assert.match(sw, /anime-tracker-v11-theme-songs-jikan-retry-1/u);
    assert.match(sw, /vendor\/opencc-js-1\.4\.1-full\.js\?v=1\.4\.1/u);
    assert.match(sw, /v11-core\.js\?v=theme-songs-1/u);
    assert.match(ui, /reloadVersion = "theme-songs-jikan-retry-1"/u);
});
